...
class SectorController extends AbstractController
{
    ...
    /**
     * @Route("/{id}/delete-checker", name="sector_delete_check", methods={"GET"})
     */
    public function deleteChecker(Sector $sector): Response
    {
        return $this->render('sector/deletecheck.html.twig', [
            'sector' => $sector,
        ]);
    }
    ...
    {   
        ...
        if ($sector->getCompanies()->isEmpty() != true) {
            $request->getSession()->getFlashBag()->add(
                'notice',
                'You can not delete this sector because some company contains it'
            );
            return $this->redirectToRoute('sector_index');
        }else if ($this->isCsrfTokenValid('delete'.$sector->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
